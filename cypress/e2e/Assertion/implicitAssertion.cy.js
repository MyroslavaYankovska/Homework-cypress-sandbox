...
it('Implicit assertions', () => {
    ...
    .eq(3).then( element => {
      ...
      expect(element.attr('class')).to.eql('success'); //deeply equel
      expect(element.attr('class')).to.eqls('success'); //deeply equel
    ...
    //expect(obj).to.eq(obj2); //fail in cypress
    expect(obj).to.eql(obj2); 
    expect(obj).to.eqls(obj2); 
   // expect(obj).to.equal(obj2);//fail in cypress
   // expect(obj).to.equals(obj2);//fail in cypress

    const obj3 = obj;

      expect(obj).to.eq(obj3);
      expect(obj).to.eql(obj3); //deeply equel
      expect(obj).to.eqls(obj3); //deeply equel
